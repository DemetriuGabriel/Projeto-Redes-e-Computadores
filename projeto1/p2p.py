...
import cryptography as DemetriuCriptografia
...
class User:
    ...
    def receive_message(self, user_socket, user_data):
        ...
        logging.info(f"Tempo de transmissão: {received_timestamp-send_timestamp} nanosegundos")

        start_time_descriptografia =  time.time_ns()

        if not DemetriuCriptografia.verify_signature(pacote_criptografado, assinatura, user_data['user_public_key']):
            raise Exception("Assinatura invalida")

        pacote_descriptografado = pickle.loads(DemetriuCriptografia.decrypted_cipher_msg(user_data['user_cipher'], pacote_criptografado)) 
        
        end_time_descriptografia =  time.time_ns()

        logging.info(f"Tempo tomado para descriptografar: {end_time_descriptografia-start_time_descriptografia} nanosegundos")
    ...
    def send_message(self, message, user_data):
        ...
        pacote_criptografado, cipher = DemetriuCriptografia.encrypt_msg(pickle.dumps(pacote), user_data['my_chave_simetrica'])
        assinatura = DemetriuCriptografia.signature(pacote_criptografado, user_data['my_private_key'])
        
        end_time_criptografia =  time.time_ns()
        logging.info(f"Tempo tomado para encriptografar: {end_time_criptografia-start_time_criptografia} nanosegundos")
    ...
    def start_handshake(self, user_socket):
        ...
        chave_ecc_privada, chave_ecc_publica = DemetriuCriptografia.generate_ecc_keys()
        chave_compartilhada = DemetriuCriptografia.generate_shared_key(chave_ecc_privada, chave_ecc_publica)
        my_chave_simetrica = DemetriuCriptografia.generate_symmetric_key_hkdf(chave_compartilhada)


        _, chave_ecc_publica_bytes = DemetriuCriptografia.transform_ecc_keys_bytes(chave_ecc_privada, chave_ecc_publica)
        ...
        self.connections_data[user_name] = {
            ...
            'user_public_key': DemetriuCriptografia.load_public_key_bytes(user_handshake_payload['public_key']),
            'my_chave_simetrica' : my_chave_simetrica,
            'user_chave_simetrica' : user_handshake_payload['chave_simetrica'],
            'user_cipher' : DemetriuCriptografia.generate_cipher_from_symmetric_key(user_handshake_payload['chave_simetrica']),
        }
